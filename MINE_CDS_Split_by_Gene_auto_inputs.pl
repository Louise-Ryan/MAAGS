...
my $species_genome_list = $ARGV[3];
chomp $species_genome_list;

unless ( open(IN4, $species_genome_list) ) {  #open the file. If it doesnt exist, exit and print an error
     print "Filename entered does not exist \n ";
	exit;
}

#store the file data as an array
my @Species_Genome_list = <IN4>;

#close the file
close IN4;

chomp(@Species_Genome_list); # remove empty lines from array
my ($Species_name, $Genome_Name);
...
foreach my $genome(@Target_genomes){
    if ($genome =~ m/(GCF+[\S\s]+_cds)/){
        $genome_id = $1;
	$genome_id =~ s/_cds//;
	if ($genome_id =~ m/(GCF_.*?_[a-zA-Z].*)/) {
	    my $genome_acc1 = $1;
	    $genome_acc1 =~ s/_[a-zA-Z].*//;
	print "looking for ".$genome_id."\n";
	foreach my $entry(@Species_Genome_list){
	    if ($entry =~ m/.*$genome_acc1.*/i){
	       my  ($species, $genome_acc2) = split (/\|/, $entry, 2);
		print $species."\n";
foreach my $target(@Target_genes) {
	...
	if ($line =~ m/(\[gene\=$gene_name\])/i) {
	    $gene_id = $1;
	    $sequence_isoforms =$sequence_isoforms.">".$genome_id."_".$gene_id."split_term"."__".$species."__".$line."\n";
	    $hits = $target; 
...
##############################################################################################################################
#Split output by gene
###############################################################################################################################

#print "Enter suffux for all output files: ";
#$suffux =  $ARGV[3];
#chomp $suffux;

    
#ask for genome file list 
#print "Enter title of text file containing target gene list:";
#$target_genes = <STDIN>;
#chomp $target_genes; # remove empty line from species_file_name
#unless ( open(TARGETGENES, $target_genes) ) {  #open the file. If it doesnt exist, exit and print an error
#     print "Filename entered does not exist \n ";
...
#ask for gene sequences file
#print "Enter title of text file containing sequences:";
#$sequence_filename = <STDIN>;
#chomp $sequence_filename;


################################################################################

#print "\n"."Do you want to split output file by genes?[Y/N]";
...
foreach my $target(@Target_genes) {
    ($gene_name2, $protein_name2, $alternative_name2) = split (/\|/, $target,3);
    print "Scanning genome for ". $gene_name2." aka ".$protein_name2." aka ".$alternative_name2."\n";
    open(IN5, $final_seq_file); #read in the CDS file for species
    my $gene_file = $gene_name2."_sequences_".$FILE_PREFIX;
    my $sequence_OUT = "";
    {local $/ = ">GCF"; #  change line delimter
     while(<IN5>) {
	 chomp;
	 my $line=$_; #store each line of the text file in $line
	 if ($line =~ m/(\[gene\=$gene_name2\])/i) {
	     if ($line =~ m/(\_\_.*\_\_)/){
		 $species = $1;
		 $species =~ s/__//g;
		 $sequence_OUT = $sequence_OUT.">".$gene_name2."_".$species." GCF".$line."\n";
	     }
	 }elsif ($line =~ m/.*\Q$protein_name2\E.*?\]/i || $line =~ m/.*\Q$alternative_name2\E.*?\]/i) { #if line matches protein name or alternative protein name
	     if ($line !~ m/.*$protein_name2[0-9].*?\]/i &&
		 $line !~ m/.*$alternative_name2[0-9].*?\]/i &&
		 $line !~ m/.*$protein_name2[\s][0-9].*?\]/i &&
		 $line !~ m/.*$alternative_name2[\s][0-9].*?\]/i) {
		 if ($line =~ m/(\_\_.*\_\_)/){
		      $species = $1;
		      $species =~ s/__//g; 
		      $sequence_OUT = $sequence_OUT.">".$gene_name2."_".$species." GCF".$line."\n";
	 ...
	 } open my $FILE, ">", $gene_file or die("Can't open file. $!");
	 print $FILE $sequence_OUT;
	 close $FILE; 
     }  
  }
}
...
my $directory = $FILE_PREFIX."_split_by_gene_files";
system("mkdir $directory");
system("mv *$FILE_PREFIX $directory");

system("mv $directory $directoryname");
